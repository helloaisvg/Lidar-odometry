...
int main(int argc, char *argv[])
{
    ...
    // 从ROS参数服务器获取配准方法参数
    int registration_method = 2; // 默认使用优化ICP
    nh.param<int>("registration_method", registration_method, 2);
    
    // 设置配准方法
    switch(registration_method) {
        case 0:
            front_end_ptr->SetRegistrationMethod(RegistrationMethod::NDT);
            ROS_INFO("Using NDT registration method");
            break;
        case 1:
            front_end_ptr->SetRegistrationMethod(RegistrationMethod::ICP);
            ROS_INFO("Using ICP registration method");
            break;
        case 2:
        default:
            front_end_ptr->SetRegistrationMethod(RegistrationMethod::OPTIMIZED_ICP);
            ROS_INFO("Using Optimized ICP registration method");
            break;
    }
