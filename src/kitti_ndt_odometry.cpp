...
int main(int argc, char** argv) {
    std::cout << "=== NDT Odometry with KITTI Data ===" << std::endl;
    ...
    // 保存结果
    std::string trajectory_file = "ndt_trajectory_kitti_" + std::to_string(sequence) + ".txt";
    if (odometry.saveTrajectory(trajectory_file)) {
        std::cout << "轨迹已保存: " << trajectory_file << std::endl;
    }
    
    // 保存全局地图
    std::string map_file = "ndt_global_map_kitti_" + std::to_string(sequence) + ".pcd";
