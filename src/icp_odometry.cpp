...
int main(int argc, char** argv) {
    ...
    if (odometry.processKITTISequence(data_path, sequence)) {
        ...
        // 保存全局地图
        std::string map_file = "gn_icp_global_map_" + std::to_string(sequence) + ".pcd";
        odometry.saveGlobalMap(map_file);
        
        // 保存全局地图截图
        std::string screenshot_file = "gn_icp_global_map_screenshot_" + std::to_string(sequence) + ".png";
        odometry.saveGlobalMapScreenshot(screenshot_file);
        
        // 可视化轨迹并录制视频
        std::cout << "\nVisualizing trajectory with video recording..." << std::endl;
        std::string video_file = "gn_icp_odometry_video_" + std::to_string(sequence) + ".mp4";
